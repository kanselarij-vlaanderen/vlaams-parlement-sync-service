...
class VP {
  ...
  async transformIncomingFlows(docs) {
    ...
    for (const doc of docs) {
      ...
      let openingDate;
      if (doc.datum) {
        openingDate = new Date(doc.datum);
      } else {
        let statuses = await VP.getStatusForFlow(pobj);
        const datum = statuses.find((status) => status.status === "aangenomen in plenaire vergadering").datum;
        openingDate = new Date(datum);
      }
