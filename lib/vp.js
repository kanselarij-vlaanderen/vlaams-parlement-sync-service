import { DOCUMENT_TYPES, DOMAIN, VP_API_CLIENT_ID, VP_API_CLIENT_SECRET, VP_ERROR_EXPIRE_TIME } from '../config';
...
class VP {
  ...
  generatePayload(
    ...
    const payload = {
      ...
      'Besluitvormingsaangelegenheid.alternatieveNaam': subcaseTitle,
          ...
          'Dossier.bestaatUit': pieces.map(
            (piece) => ({
              ...
              'Stuk.isVoorgesteldDoor': piece
                ...
                .filter(getRedundantFileFilter(piece))
                .map((file) => {
...
const getRedundantFileFilter = piece => file => {
  /**
   * For decreten and memories, we only want to send the signed pdf.
   */
  return !(
    file.isPdf &&
    [DOCUMENT_TYPES.DECREET, DOCUMENT_TYPES.MEMORIE].includes(piece.type.uri) &&
