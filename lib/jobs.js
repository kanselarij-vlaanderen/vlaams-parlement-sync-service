import { sparqlEscapeString, sparqlEscapeUri, sparqlEscapeDateTime, sparqlEscapeInt, sparqlEscapeBool, uuid } from 'mu';
...
async function getNextScheduledJob() {
  ...
  SELECT ?uri ?id ?status ?created ?modified ?agendaitem ?piece ?comment ?submitter ?isComplete
  WHERE {
    GRAPH ${sparqlEscapeUri(JOB.GRAPH)} {
      VALUES ?status {
