    ...
    for (const file of doc.files) {
      ...
      if (representations.length === 2 && pdf && word) {
        ...
        for (const representation of representations) {
          const { pieceUri } = await createDocumentContainerAndPiece(file.fileNameWithoutExtension, file.documentType);
  ...
  for (const doc of docs) {
    ...
    if (!await pobjIsKnown(pobj)) {
      ...
      if (result) {
        ...
        // Create subcase
        const subcaseResult = await createSubcase(title,shortTitle, openingDate, subcaseType, agendaItemType, decisionmakingFlow);
