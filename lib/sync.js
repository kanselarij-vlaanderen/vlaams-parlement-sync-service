...
>>>>>>> 3a960f31d5cfb5834e62f0f6ad859047cc50c4aa
  getLatestSubcase
  ...
  for (const doc of docs) {
    ...
    if (!await pobjIsKnown(pobj)) {
      ...
      if (result) {
        ...
        // const latestSubcase = await getLatestSubcase(decisionmakingFlow);
        // Find mandatees of previous subcase, if any
        const { uri: latestSubcase } = await getLatestSubcase(decisionmakingFlow);
        let latestSubcaseGovernmentAreas = [];
        if (latestSubcase) {
          latestSubcaseGovernmentAreas = await getLatestGovernmentAreas(latestSubcase);
        }
        // Create subcase
        const subcaseResult = await createSubcase(title, shortTitle, subcaseType, agendaItemType, decisionmakingFlow, latestSubcase, latestSubcaseGovernmentAreas, isSudo);
