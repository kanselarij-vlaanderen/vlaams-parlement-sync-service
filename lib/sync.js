...
import { createEmailToVP, createEmailOnSyncFailure } from './email';
...
async function syncSubmittedFlows(isSudo) {
  try {
    ...
    console.log('Something went wrong while syncing submitted flows');
    console.trace(error);
    await createEmailOnSyncFailure(
      "Something went wrong while syncing submitted flows in VP service",
      `environment: ${KALEIDOS_HOST_URL}, detail of error: ${error?.message || "no details available"}`
    );
  }
}
...
async function syncIncomingFlows(isSudo) {
  try {
    ...
    accessLevelMapping[DOCUMENT_TYPES.VERWIJZINGSFICHE] =
      ACCESS_LEVELS.INTERN_SECRETARIE;
    ...
    const createPieces = async (
      ...
      for (const file of doc.files) {
        ...
        const pdf = representations.find(
          (r) => r.mimeType === "application/pdf"
        );
        const word = representations.find(
          (r) =>
            r.mimeType ===
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        );
        const comment = representations
          .map((r) => r.comment?.trim())
          .filter((c) => c?.length)
          .join(" - ");
        const submissionActivityUri = await createSubmissionActivity(
          subcaseUri,
          isSudo
        );
        ...
        if (representations.length === 2 && pdf && word) {
          ...
          pieces.push({
            uri: pieceUri,
            files: [
              { pfls: pdf.pfls, uri: pdfUri },
              { pfls: word.pfls, uri: wordUri },
            ],
          });
          ...
          pieces.push({
            uri: pieceUri,
            files: [{ pfls: pdf.pfls, uri: fileUri }],
          });
        } else {
          ...
          for (const representation of representations) {
            ...
            pieces.push({
              uri: pieceUri,
              files: [{ pfls: representation.pfls, uri: fileUri }],
            });
          }
        }
      }
    ...
    for (const doc of docs) {
      ...
      if (!(await pobjIsKnown(pobj))) {
        ...
        } = await createCaseDecisionmakingFlowAndSubcase(
          title,
          shortTitle,
          openingDate,
          subcaseType,
        ...
        const parliamentFlowUri = await createParliamentFlow(
          pobj,
          decisionmakingFlowUri,
          PARLIAMENT_FLOW_STATUSES.RECEIVED,
          isSudo
        );
        const parliamentSubcase = await createParliamentSubcase(
          parliamentFlowUri,
          isSudo
        );
        ...
        await VP.notifyReceivedDocument(
          VP.generateNotificationPayload(
            { pobj, uri: decisionmakingFlowUri, case: caseUri },
            pieces
          )
        );
      } else {
        ...
        const { parliamentSubcase } = await getParliamentSubcaseFromPobj(
          pobj,
          isSudo
        );
        ...
        if (result) {
          ...
          const { uri: latestSubcase } = await getLatestSubcase(
            decisionmakingFlow
          );
          const latestSubcaseGovernmentAreas = latestSubcase
            ? await getLatestGovernmentAreas(latestSubcase)
            : [];
          ...
          if (subcaseType === SUBCASE_TYPES.BEKRACHTIGING_VLAAMSE_REGERING) {
            const { uri: latestDefinitiveSubcase } = await getLatestSubcase(
              decisionmakingFlow,
              SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING
            );
            const mandatees = latestDefinitiveSubcase
              ? await getCalculatedMandatees(latestDefinitiveSubcase)
              : [];
            // add PM at all costs
          ...
          // Create subcase
          const subcaseResult = await createSubcase(
            title,
            shortTitle,
            subcaseType,
            agendaItemType,
            decisionmakingFlow,
            definitiveMandatees,
            latestSubcaseGovernmentAreas,
            isSudo
          );
          subcase = subcaseResult.subcaseUri;
          subcaseId = subcaseResult.subcaseId;

          _case = await getCaseFromDecisionmakingFlow(
            decisionmakingFlow,
            isSudo
          );
          decisionmakingFlowId = await getDecisionmakingFlowId(
            decisionmakingFlow,
            isSudo
          );
        }
        ...
        (e.g DOCX and PDF) we need to put them into a single piece. */
          ...
          for (const representation of representations) {
            ...
            if (representation.uri && (await fileIsKnown(representation.uri))) {
              ...
              if (await pieceIsOnFinalMeeting(pieceUri)) {
                ...
                const fileUri = await saveFile(representation);
                await moveDerivedAndSourceLinks(
                  representation.uri,
                  fileUri,
                  isSudo
                );
          ...
          const pdf = validRepresentations.find(
            (r) => r.mimeType === "application/pdf"
          );
          const word = validRepresentations.find(
            (r) =>
              r.mimeType ===
              "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          );
          const comment = validRepresentations
            .map((r) => r.comment?.trim())
            .filter((c) => c?.length)
            .join(" - ");
        ...
        if (pieces.length) {
          await VP.notifyReceivedDocument(
            VP.generateNotificationPayload(
              { pobj, uri: decisionmakingFlow, case: _case },
              pieces
            )
          );
        }
      ...
      await createEmailToVP(
        "Nieuwe documenten van het Vlaams Parlement",
        `Het Vlaams Parlement heeft nieuwe documenten doorgestuurd.
      
Voor het dossier "${shortTitle}" zijn ze beschikbaar op ${caseUrl} in de procedurestap ${subcaseUrl}.`
      );
    }
  } catch (error) {
    console.log('Something went wrong while syncing incoming flows');
    console.trace(error);
    await createEmailOnSyncFailure(
      "Something went wrong while syncing incoming flows in VP service",
      `environment: ${KALEIDOS_HOST_URL}, detail of error: ${error?.message || "no details available"}`
    );
  }
}
