  ...
  for (const doc of docs) {
    ...
    if (!await pobjIsKnown(pobj)) {
      ...
      if (result) {
        ...
        if (latestSubcase) {
          latestSubcaseGovernmentAreas = await getLatestGovernmentAreas(latestSubcase.uri);
        }
        // Create subcase
        const subcaseResult = await createSubcase(title, shortTitle, subcaseType, agendaItemType, decisionmakingFlow, latestSubcaseGovernmentAreas, isSudo);
