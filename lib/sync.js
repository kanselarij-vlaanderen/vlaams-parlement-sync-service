...
import { createEmailOnSyncSuccess, createEmailOnSyncFailure } from './email';
...
async function syncSubmittedFlows(isSudo) {
  try {
    ...
    console.log('VP SYNC FAILURE while syncing submitted flows');
    console.trace(error);
    await createEmailOnSyncFailure(
      "VP SYNC FAILURE while syncing submitted flows",
...
async function syncIncomingFlows(isSudo) {
  try {
    ...
    for (const doc of docs) {
      ...
      await createEmailOnSyncSuccess(
...
Voor het dossier "${shortTitle}" zijn ze beschikbaar op ${caseUrl} in de procedurestap ${subcaseUrl}.`
    ...
    console.log('VP SYNC FAILURE while while syncing incoming flows');
    console.trace(error);
    await createEmailOnSyncFailure(
      "VP SYNC FAILURE while syncing incoming flows",
      `environment: ${KALEIDOS_HOST_URL}
