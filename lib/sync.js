...
async function syncIncomingFlows() {
  ...
  const createPieces = async (doc, parliamentSubcaseUri, subcaseUri) => {
    ...
    for (const file of doc.files) {
      ...
      if (representations.length === 2 && pdf && word) {
        ...
        const { pieceUri } = await createDocumentContainerAndPiece(doc.shortTitle, file.documentType);
        ...
        pieceUris.push(pieceUri);
      } else if (representations.length === 1 && pdf) {
        // Create a singe piece with the shortTitle as its name
        const { pieceUri } = await createDocumentContainerAndPiece(doc.shortTitle, file.documentType);
        ...
        for (const representation of representations) {
          const { pieceUri } = await createDocumentContainerAndPiece(representation.fileName, file.documentType);
    ...
    await createSubmissionActivity(subcaseUri, pieceUris);
  }
  ...
  for (const doc of docs) {
    ...
    if (!await pobjIsKnown(pobj)) {
      ...
      if (pieceUris.length) {
        await createSubmissionActivity(subcase, pieceUris);
      }
    }
