...
import { PARLIAMENT_FLOW_STATUSES, VP_GRAPH_URI } from "../config";
...
WHERE {
  VALUES (?flowUri ?parliamentId) {
    ${flows
      .map(
        (flow) =>
          `(${flow.uri ? sparqlEscapeUri(flow.uri) : "UNDEF"} ${
            flow.parliamentId ? sparqlEscapeString(flow.parliamentId) : "UNDEF"
          })`
      )
      .join("\n    ")}
  }
