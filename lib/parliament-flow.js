 ...
 *
 * @param {string} uri
 * @returns {Promise<Object>} Object containing a pobj field
 */
async function getPobjFromParliamentflow(uri) {
  const queryString = `${prefixHeaderLines.parl}

SELECT DISTINCT ?pobj
FROM ${sparqlEscapeUri(VP_GRAPH_URI)}
WHERE {
  ${sparqlEscapeUri(uri)} a parl:Parlementaireaangelegenheid ;
    parl:id ?pobj .
}`;
  const response = await querySudo(queryString);
