...
import { escapedGraphs, prefixHeaderLines, RESOURCE_BASE } from "../constants";
...
WHERE {
  {
    ...
    WHERE {
      ...
      GRAPH ${escapedGraphs.kanselarij} {
...
ASK {
  GRAPH ${escapedGraphs.kanselarij} { ${sparqlEscapeUri(file)} a nfo:FileDataObject . }
}`;
  const response = await querySudo(queryString);
  return response?.boolean;
}
