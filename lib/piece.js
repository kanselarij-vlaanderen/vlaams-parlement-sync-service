...
async function retrieveDocumentContainerPosition(subcaseUri) {

  const queryString = `${prefixHeaderLines.ext}
${prefixHeaderLines.prov}
${prefixHeaderLines.dossier}
${prefixHeaderLines.schema}

SELECT (MAX(?position) as ?maxPosition) FROM ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} WHERE {
  ?submissionActivity ext:indieningVindtPlaatsTijdens ${sparqlEscapeUri(subcaseUri)}  ;
  ...
  return parsed[0]?.maxPosition ? parseInt(parsed[0].maxPosition) + 1 : 1;
}

/**
 * [SUDO QUERY]
 */
async function createDocumentContainerAndPiece(title, documentType, accessLevel=ACCESS_LEVELS.INTERN_OVERHEID, isSudo, subcaseUri, submissionActivityUri) {
  const now = new Date();
  ...
  const documentContainerPosition = await retrieveDocumentContainerPosition(subcaseUri);
...
INSERT DATA {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
    ${sparqlEscapeUri(documentContainerUri)} a dossier:Serie ;
      ...
      schema:position ${sparqlEscapeInt(documentContainerPosition)} ;
    ...
    ${sparqlEscapeUri(submissionActivityUri)} prov:generated ${sparqlEscapeUri(pieceUri)} .
  }
}`;
  const updateFunc = isSudo ? updateSudo : update;
...
WHERE {
  ...
  INSERT DATA {
    ...
    // the link to the piece must be replaced as well, but only if it existed before
    const pieceUpdateString = `${prefixHeaderLines.prov}
  DELETE {
    GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
      ?piece prov:value ${sparqlEscapeUri(oldFileUri)} .
    }
  }
  INSERT {
    GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
      ?piece prov:value ${sparqlEscapeUri(newFileUri)} .
    }
  } WHERE {
    GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
      ?piece prov:value ${sparqlEscapeUri(oldFileUri)} .
    ...
    await updateFunc(pieceUpdateString);
}
