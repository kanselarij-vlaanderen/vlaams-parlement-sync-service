...
import { ACCESS_LEVELS, DECISION_RESULT_CODES, KANSELARIJ_GRAPH_URI, VP_GRAPH_URI } from '../config';
...
WHERE {
  ...
  const updateString = `
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} { ${sparqlEscapeUri(fileUri)} ?p ?o }
} ;
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} { ?s ?p ${sparqlEscapeUri(fileUri)} }
} ;
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(VP_GRAPH_URI)} { ${sparqlEscapeUri(fileUri)} ?p ?o }
} ;
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(VP_GRAPH_URI)} { ?s ?p ${sparqlEscapeUri(fileUri)} }
} ;
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} { ${sparqlEscapeUri(physicalFile)} ?p ?o }
} ;
DELETE WHERE {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} { ?s ?p ${sparqlEscapeUri(physicalFile)} }
}`;
  await updateSudo(updateString);
}

/**
 * [SUDO QUERY]
 */
async function createDocumentContainerAndPiece(title, documentType) {
  const now = new Date();
...
INSERT DATA {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
    ...
    ${sparqlEscapeUri(pieceUri)} a dossier:Stuk ;
      ...
      dct:created ${sparqlEscapeDateTime(now)} ;
      besluitvorming:vertrouwelijkheidsniveau ${sparqlEscapeUri(ACCESS_LEVELS.INTERN_OVERHEID)} .
  }
}`;
 ...
 * [SUDO QUERY]
 */
async function createFile(filePath, fileName, fileSize, extension, mimeType) {
  const now = new Date();
 ...
 * [SUDO QUERY]
 */
async function linkFileToPiece(fileUri, pieceUri) {
  const queryString = `${prefixHeaderLines.prov}
 ...
 * [SUDO QUERY]
 */
async function linkDerivedFileToSourceFile(derivedUri, sourceUri) {
  const queryString = `${prefixHeaderLines.prov}
