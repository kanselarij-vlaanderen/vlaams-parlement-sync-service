...
function findPiecesOfType(pieces, pieceType) {
  return pieces.filter((piece) => { return piece.type.uri === pieceType; });
}
        ...
        for (const subcase in subcasesWithPieces) {
          if (subcasesWithPieces.hasOwnProperty(subcase)) {
            ...
            if (subcaseWithPieces.subcaseType === requirement.subcaseType) {
              ...
              for (const requiredPiece of requirement.requiredPieces) {
                let piecesOfType = findPiecesOfType(subcaseWithPieces.pieces, requiredPiece.pieceType);
                if (piecesOfType && piecesOfType.length) {
                  for (const piece of piecesOfType) {
                    let hasRequiredPiece = requiredPieces.some((element) => {
                      return element.id === piece.id;
                    });
                    if (!hasRequiredPiece) {
