...
async function getPieceUris(agendaitemUri) {
  ...
  debugger
  console.log('logging:' + subcaseType);
  console.log('logging:' + documentTypes.indexOf(DOCUMENT_TYPES.DECREET));
  console.log('logging:' + documentTypes);
...
async function getRequiredPieces (agendaitemUri, pieces) {
          ...
          if (subcasesWithPieces.hasOwnProperty(subcase)) {
            ...
            if (subcaseWithPieces.subcaseType === requirement.subcaseType) {
              for (const requiredPiece of requirement.requiredPieces) {
                ...
                if (!requiredPieces[subcaseName]) {
                  requiredPieces[subcaseName] = {};
                }
                if (!requiredPieces[subcaseName][pieceTypeLabel]) {
                  requiredPieces[subcaseName][pieceTypeLabel] = [];
                }
                let obj = {};
                obj[fileType] = true;
                requiredPieces[subcaseName][pieceTypeLabel].push(obj);
              }
            }
          }
        }
  ...
  for (const subcaseName in requiredPieces) {
    if (requiredPieces.hasOwnProperty(subcaseName)) {
      for (const pieceTypeLabel in requiredPieces[subcaseName]) {
        if (requiredPieces[subcaseName].hasOwnProperty(pieceTypeLabel)) {
          requiredPieces.push({
            subcaseName: subcaseName === 'default' ? undefined : subcaseName,
            type: {
              label: pieceTypeLabel,
            },
            files: requiredPieces[subcaseName][pieceTypeLabel]
          })
        }
      }
    }
