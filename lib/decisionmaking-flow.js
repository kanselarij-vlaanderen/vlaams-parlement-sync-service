...
WHERE {
  ...
  VALUES (?subcaseType ?documentType) {
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.BESLISSINGSFICHE)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.DECREET)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.MEMORIE)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.NOTA)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.ADVIES)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.ADVIES_IF)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.BIJLAGE)})
    (${sparqlEscapeUri(SUBCASE_TYPES.DEFINITIEVE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.BEGROTINGSAKKOORD)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.BESLISSINGSFICHE)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.DECREET)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.MEMORIE)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.NOTA)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.ADVIES)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.ADVIES_IF)})
    (${sparqlEscapeUri(SUBCASE_TYPES.PRINCIPIELE_GOEDKEURING)} ${sparqlEscapeUri(DOCUMENT_TYPES.BEGROTINGSAKKOORD)})
  }
...
INSERT DATA {
  GRAPH ${sparqlEscapeUri(KANSELARIJ_GRAPH_URI)} {
    ${sparqlEscapeUri(caseUri)} a dossier:Dossier ;
      ...
      dct:created ${sparqlEscapeDateTime(openingDate)} ;
 ...
 * @param {string} agendaItemType
 */
async function createSubcase(title, shortTitle, openingDate, subcaseType, agendaItemType, decisionmakingFlowUri) {
  const subcaseId = uuid();
  const subcaseUri = `${RESOURCE_BASE}procedurestap/${subcaseId}`;
