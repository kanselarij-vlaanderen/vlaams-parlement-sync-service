 ...
 * @param {string} subcaseType
 * @param {string} agendaItemType
 * ADD OPTIONAL PARAM FOR LIST
 */
async function createSubcase(title, shortTitle, subcaseType, agendaItemType, decisionmakingFlowUri, governmentAreas, isSudo) {
  const subcaseId = uuid();
  const subcaseUri = `${RESOURCE_BASE}procedurestap/${subcaseId}`;
  const now = new Date();
  console.log("GOVERNMENTAREAS:", governmentAreas);
